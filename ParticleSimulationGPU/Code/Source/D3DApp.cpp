...
HRESULT D3DApp::DetectComputeShaderSupport()
{
	D3D11_FEATURE_DATA_D3D10_X_HARDWARE_OPTIONS options; 
	HRESULT hr = mDevice->CheckFeatureSupport( D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS, &options, sizeof(D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS) ); 

	if( !options.ComputeShaders_Plus_RawAndStructuredBuffers_Via_Shader_4_x )
	{ 
		MessageBoxA( NULL, "Compute Shaders are not supported on your hardware", "Unsupported Hardware", MB_OK );
		return E_FAIL; 
	}
	else
		return S_OK;
...
HRESULT D3DApp::InitializeDirectX11()
{
	...
	//Check for CS support
	hr = DetectComputeShaderSupport();
